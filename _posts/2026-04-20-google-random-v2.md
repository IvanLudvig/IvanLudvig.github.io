...
Right-shifting by 11 bits, instead of 12, gives us one extra bit of entropy, making it 53 bits total, as well as a statistically better, more uniform distribution.
...
```
`r` maps to `x` when it falls in the half-open interval `[lo, hi)`, where `lo = normalize(x) * 2^53` and `hi = normalize(x+1) * 2^53`. Since `r` is an integer, the inclusive bounds are `[ceil(lo), ceil(hi) - 1]`. `lo` is generally fractional, so the smallest valid integer is `ceil(lo)`. Using `floor` would include a value that maps to `x - 1`. Similarly, `ceil(hi) - 1` is used because `ceil(hi)` would map to `x + 1`. In the code:
...
Quantum computing? No, telemetry. The paradox was caused by a call to `Math.random()` triggered by an event listener on selection, applied to the whole page. It's connected to Google's telemetry and `Math.random()` is used to collect just 5% of the data.

Moreover, I've discovered that when using Chrome without a Google profile logged in, every click of the "Generate" button runs 2 extra calls of `Math.random()`. Similarly, some other clicks also trigger 2 calls. This is connected to telemetry too, which requires a 64-bit ID. Since each call produces just 53 bits of entropy, two calls are required.
